var XMPPVideoRoom = (function () {
  ...
  var XMPPVideoRoom = function XMPPVideoRoom(xmppUrl, srvurl, bus) {
    ...
    this.srvurl =
      srvurl || "//" + window.location.hostname + ":" + window.location.port;
    this.sessionList = {};
    this.bus = bus;
    this.connection = {};
  };
    ...
    return new Promise((resolve, reject) => {
      if (this.connection.hasOwnProperty(key)) {
        ...
        var connection = new Strophe.Connection(
          "https://" + this.xmppUrl + "/http-bind?room=" + roomId
        );
  ...
  XMPPVideoRoom.prototype.join = function (roomId, url, username, password) {
    this.getConnection(roomId, username).then((connection) => {
      ...
      connection.addHandler(
        this.OnJingle.bind(this, connection, roomId, username, url),
        "urn:xmpp:jingle:1",
        "iq",
        "set",
        null,
      ...
      var extPresence = Strophe.xmlElement(
        "nick",
      ...
      connection.muc.join(
        ...
        username,
        null,
        this.OnPresence.bind(this, connection, roomId),
        null,
        password,
        null,
        extPresence
  ...
  XMPPVideoRoom.prototype.leave = function (roomId, username) {
    ...
    if (!found) {
      ...
      this.getConnection(roomId, username).then((connection) => {
        var sessionname =
          "kicker" + Math.floor(Math.random() * 1000000).toString();
        ...
        connection.muc.kick(roomUrl, username, "Unknown session", () =>
          this.emitPresence(roomId + "/" + username, "out")
        );
  ...
  /*
	...
	*/
  XMPPVideoRoom.prototype.onReceiveCandidate = function (
    connection,
    roomid,
    name,
    answer,
    ...
    candidateList.forEach(function (candidate) {
      ...
      var id = connection.sendIQ(
        ...
        (error) => {
          console.log(
            "############transport-info error sid:" + sid + " error:" + error
          );
        }
      );
    });
    ...
    var id = connection.sendIQ(
      ...
      (error) => {
        console.log(
          "############session-accept error sid:" + sid + " error:" + error
        );
      }
    );
  };
  ...
  /*
	...
	*/
  XMPPVideoRoom.prototype.onCall = function (
    connection,
    roomid,
    name,
    iq,
  ...
  /*
	...
	*/
  XMPPVideoRoom.prototype.OnJingle = function (
    connection,
    roomid,
    name,
    url,
    ...
    if (action === "session-initiate") {
      ...
      if (!isP2P) {
        ...
        fetch(method, {
          method: "POST",
          body: JSON.stringify({ type: "offer", sdp: sdp.raw }),
        })
          .then((response) => response.json())
          .then((response) =>
          ...
          transports.each((idx, transport) => {
            ...
            candidates.each((idx, candidate) => {
              ...
              if (this.sessionList[sid].earlyCandidates) {
                ...
                console.log(
                  "===> webrtc candidate :" + JSON.stringify(candidate)
                );
                var method = this.srvurl + "/api/addIceCandidate?peerid=" + sid;
      ...
      connection.sendIQ(ack);
      console.log(
        "<=== xmpp session-terminate sid:" +
          sid +
          " reason:" +
          jingle.querySelector("reason").textContent
      );
      this.leaveSession(sid);
    }
  ...
  XMPPVideoRoom.prototype.OnPresence = function (connection, roomid, pres) {
    ...
    const xElement = pres.getElementsByTagNameNS(
    ...
    if (mucUserItem) {
      msg +=
        " jid:" +
        mucUserItem.getAttribute("jid") +
        " role:" +
        mucUserItem.getAttribute("role") +
        " affiliation:" +
...
else window.XMPPVideoRoom = XMPPVideoRoom;
