...
void OggTrackTable::add(OggTrack* newTrack) {
  OggTrack* existingTrack
    = (OggTrack*)fTable->Add(reinterpret_cast<char const*>(newTrack->trackNumber), newTrack);
  delete existingTrack; // if any
}

OggTrack* OggTrackTable::lookup(u_int32_t trackNumber) {
  return (OggTrack*)fTable->Lookup(reinterpret_cast<char const*>(trackNumber));
}
...
FramedSource* OggDemux::newDemuxedTrack(u_int32_t& resultTrackNumber) {
  ...
  fDemuxedTracksTable->Add(reinterpret_cast<char const*>(resultTrackNumber), trackSource);
  return trackSource;
}
...
OggDemuxedTrack* OggDemux::lookupDemuxedTrack(u_int32_t trackNumber) {
  return (OggDemuxedTrack*)fDemuxedTracksTable->Lookup(reinterpret_cast<char const*>(trackNumber));
}
