...
::getStreamParameters(unsigned clientSessionId,
  ...
  fClientRTCPSourceRecords->Add(reinterpret_cast<char const*>(clientSessionId), source);
}
...
void PassiveServerMediaSubsession::startStream(unsigned clientSessionId,
  ...
  if (fRTCPInstance != NULL) {
    ...
    RTCPSourceRecord* source = (RTCPSourceRecord*)(fClientRTCPSourceRecords->Lookup(reinterpret_cast<char const*>(clientSessionId)));
    if (source != NULL) {
...
void PassiveServerMediaSubsession::deleteStream(unsigned clientSessionId, void*& /*streamToken*/) {
  ...
  if (source != NULL) {
    ...
    fClientRTCPSourceRecords->Remove(reinterpret_cast<char const*>(clientSessionId));
    delete source;
  }
}
