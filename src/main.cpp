...
int main(int argc, char* argv[]) {
    ...
    while ((c = getopt(argc, argv,
        ...
        switch (c) {
            ...
            case 'a':
                audioLayer =
                        optarg ? (webrtc::AudioDeviceModule::AudioLayer)atoi(
                                         optarg)
            ...
            case 'R':
                ...
                localWebrtcUdpPortRange =
                        optarg ? optarg : defaultWebrtcUdpPortRange;
                std::cout << localWebrtcUdpPortRange
                          << " [-R defaultWebrtcUdpPortRange]" << std::endl;
                break;
            case 'h':
            default:
                std::cout << argv[0]
                          ...
                          << std::endl;
                std::cout << argv[0]
                          << " [-H http port] [-s[externel stun address]] [-t "
                             "[username:password@]turn_address] -[v[v]] "
                             "[url1]...[urln]"
                          << std::endl;
                ...
                std::cout << "\t -H hostname:port   : HTTP server binding "
                             "(default "
                          << httpAddress << ")" << std::endl;
                std::cout << "\t -w webroot         : path to get files"
                          << std::endl;
                std::cout << "\t -c sslkeycert      : path to private key and "
                             "certificate for HTTPS"
                          << std::endl;
                std::cout << "\t -N nbthreads       : number of threads for "
                             "HTTP server"
                          << std::endl;
                std::cout << "\t -A passwd          : password file for HTTP "
                             "server access"
                          << std::endl;
                std::cout << "\t -D authDomain      : authentication domain "
                             "for HTTP server access (default:mydomain.com)"
                          << std::endl;
                ...
                std::cout << "\t -s[stun_address]                   : use an "
                          ...
                          << stunurl << " , -:means no STUN)" << std::endl;
                std::cout << "\t -t[username:password@]turn_address : use an "
                ...
                std::cout << "\t -a[audio layer]                    : spefify "
                             "audio capture layer to use (default:"
                          << audioLayer << ")" << std::endl;

                std::cout << "\t -n name -u videourl -U audiourl    : register "
                             "a stream with name using url"
                          << std::endl;
                std::cout << "\t [url]                              : url to "
                             "register in the source list"
                          << std::endl;
                std::cout << "\t -C config.json                     : load "
                             "urls from JSON config file"
                          << std::endl;

                std::cout << "\t -R [Udp port range min:max]        : Set the "
                             "webrtc udp port range (default 0:65534)"
                          << std::endl;
        ...
        try {
            ...
            if (localstunurl != NULL) {
                ...
                if (server_socket) {
                    ...
                    std::cout << "STUN Listening at " << server_addr.ToString()
                              << std::endl;
                }
            }
            ...
            if (localturnurl != NULL) {
                ...
                turnserver.reset(
                        new cricket::TurnServer(rtc::Thread::Current()));
                ...
                rtc::AsyncSocket* tcp_server_socket =
                        thread->socketserver()->CreateAsyncSocket(AF_INET,
                                                                  SOCK_STREAM);
                ...
                std::cout << "TURN external addr:"
                          << external_server_addr.ToString() << std::endl;
